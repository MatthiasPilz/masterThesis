...
// 		printf ( "final i = %d\n\n", i );
            ...
            sprintf ( out, "../data/%s_convergence_M%d/%s/%s/CFL_%.8f/N_%d/error_Nt_%08d.txt", par.project, par.M, par.shape, par.scheme, par.CFL, par.N, Nt_evolve );
            FILE *f_out = fopen ( out, "w" );

            for ( i = 0; i < par.N; ++i ) { 				/// L1-norm
                norm_L1 = fabs ( I[i] - F[i] );
                fprintf ( f_out, "%.15f, %.15f\n", (i+0.5)*par.dx, norm_L1 );
            }
